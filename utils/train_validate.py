...
def train(dataloaders, models, optimizer, scheduler, args, logger):
    ...
    cur_iter = 0
    pos_ratio = 0
    patch_size = model.module.config.patch_size if isinstance(model, DataParallel) or isinstance(model,
                                                                                                 DDP) else model.config.patch_size
    ...
    for epoch in range(args.epochs):
        ...
        for i, (img1, img2, dis_gene, float_gene, grade) in enumerate(train_loader):
            img1, img2, dis_gene, float_gene, grade = img1.cuda(non_blocking=True), img2.cuda(non_blocking=True), \
                                                      dis_gene.cuda(non_blocking=True), float_gene.cuda(
                non_blocking=True), \
            ...
            region_loss = args.lambda_region * global_local(global_region_features, pos_region_features,
                                                            neg_region_features)
            # float gene guidance
            ...
            dis_gene_loss = args.lambda_dis_gene * discrete_gene_guidance(global_gene_features, pos_gene_features,
                                                                          neg_gene_features, dis_gene)
